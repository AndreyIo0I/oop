...
bool CRemoteControl::Info(std::istream & args/*args*/)
{
...
bool CRemoteControl::SelectChannel(std::istream & input)
{
	string channel;
	input >> channel;
	...
	try
	{
		int number = stoi(channel);
		if (m_tv.SelectChannel(number))
		{
			cout << "Selected channel: " << to_string(m_tv.GetChannel()) << "\n";
			return true;
		}
	}
	catch (exception & e) {
		cout << "Input must be a decimal number\n";
	}
